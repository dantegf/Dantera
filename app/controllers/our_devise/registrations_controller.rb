class OurDevise::RegistrationsController < Devise::RegistrationsController
  protected
    ...
    if cookies[:item].present? && cookies[:name].present?
      ...
      item_hash = JSON.parse(cookies["item"]).with_indifferent_access
      code = SecureRandom.hex (3)
      item = Item.new(user_id: current_user.id, name: "item", description:item_hash[:description], code: code, message:item_hash[:message], reward:item_hash[:reward], contact:item_hash[:contact], photo:item_hash[:photo])
      item.save!
    end
      items_path
  end
